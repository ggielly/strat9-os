...
pub fn posix_oflags_to_strat9(posix: u32) -> OpenFlags {
    ...
    if posix & O_CREAT != 0    { out |= OpenFlags::CREATE; }
    if posix & O_TRUNC != 0    { out |= OpenFlags::TRUNCATE; }
    if posix & O_APPEND != 0   { out |= OpenFlags::APPEND; }
    if posix & O_DIRECTORY != 0 { out |= OpenFlags::DIRECTORY; }
    if posix & O_EXCL != 0     { out |= OpenFlags::EXCL; }
    if posix & O_NONBLOCK != 0 { out |= OpenFlags::NONBLOCK; }
    if posix & O_NOFOLLOW != 0 { out |= OpenFlags::NOFOLLOW; }
    if posix & O_NOCTTY != 0   { out |= OpenFlags::NOCTTY; }
    if posix & O_SYNC != 0     { out |= OpenFlags::SYNC; }
