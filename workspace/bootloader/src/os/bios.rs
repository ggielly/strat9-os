...
// Import from bios submodules
pub mod bios;
pub use self::bios::{DiskBios, ThunkData, VideoModeIter, Vga, VgaTextColor};
use self::bios::memory_map::memory_map;
...
pub static VGA: Mutex<Vga> = Mutex::new(unsafe { Vga::new(VGA_ADDR, 80, 25) });
...
impl Os for OsBios {
    ...
    fn boot_disk(&self) -> Option<DiskBios> {
        match u8::try_from(self.boot_disk) {
            Ok(disk_num) => Some(DiskBios::new(disk_num, self.thunk13)),
            Err(_) => None,
        }
    }
...
pub unsafe extern "C" fn start(
    ...
    unsafe {
        ...
        {
            let mut com1 = bios::serial::COM1.lock();
            com1.init();
            com1.write(b"SERIAL\n");
        }
