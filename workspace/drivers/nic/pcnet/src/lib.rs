...
use net_core::{NetworkDevice, NetError};
...
impl PcnetDevice {
    pub unsafe fn new(pci_dev: crate::arch::x86_64::pci::PciDevice) -> Result<Self, &'static str> {
        let io_base = match pci_dev.read_bar(0) {
            Some(crate::arch::x86_64::pci::Bar::Io(addr)) => addr as u16,
        ...
        for i in 0..RX_BUFFERS_COUNT {
            ...
            rx_buffers[i] = crate::memory::phys_to_virt(rx_phys[i]) as *mut u8;
        }
        ...
        for i in 0..TX_BUFFERS_COUNT {
            ...
            tx_buffers[i] = crate::memory::phys_to_virt(tx_phys[i]) as *mut u8;
        }

        let rx_des_frame = crate::memory::allocate_dma_frame()
            .ok_or("Failed to allocate RX descriptors")?;
        let rx_des_phys = rx_des_frame.start_address();
        let rx_des = crate::memory::phys_to_virt(rx_des_phys) as *mut u8;
        core::ptr::write_bytes(rx_des, 0, RX_BUFFERS_COUNT * DESC_LEN);

        let tx_des_frame = crate::memory::allocate_dma_frame()
            .ok_or("Failed to allocate TX descriptors")?;
        let tx_des_phys = tx_des_frame.start_address();
        let tx_des = crate::memory::phys_to_virt(tx_des_phys) as *mut u8;
    ...
    fn init(&mut self) {
        ...
        let init_struct_frame = crate::memory::allocate_dma_frame().unwrap();
        let init_phys = init_struct_frame.start_address();
        let init_virt = crate::memory::phys_to_virt(init_phys) as *mut u8;
        ...
        csr_0 |= 1 << CSR0_INIT as usize;
        ports.write_csr(0, csr_0);

        core::hint::spin_loop();

        let mut csr_0 = ports.read_csr(0);
        csr_0 |= 1 << CSR0_STRT as usize;
        ports.write_csr(0, csr_0);
...
pub fn init() {
    ...
    let candidates = crate::arch::x86_64::pci::probe_all(crate::arch::x86_64::pci::ProbeCriteria {
        ...
        device_id: None,
        class_code: Some(crate::arch::x86_64::pci::class::NETWORK),
        subclass: Some(crate::arch::x86_64::pci::net_subclass::ETHERNET),
        prog_if: None,
    });
    ...
    for pci_dev in candidates.into_iter() {
        ...
        match unsafe { PcnetDevice::new(pci_dev) } {
            Ok(device) => {
                ...
                let iface = crate::hardware::nic::register_device(arc);
                log::info!("PCnet: Registered as {}", iface);
            }
            Err(e) => {
    ...
    let devices = PCNET_DEVICES.lock();
    log::info!("[PCnet] Found {} device(s)", devices.len());
}
