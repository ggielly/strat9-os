...
impl Rtl8139Device {
    pub unsafe fn new(pci_dev: crate::arch::x86_64::pci::PciDevice) -> Result<Self, &'static str> {
        ...
        let rx_frame = crate::memory::allocate_dma_frame()
            .ok_or("Failed to allocate RX buffer")?;
        let rx_phys = rx_frame.start_address();
        let rx_buffer = crate::memory::phys_to_virt(rx_phys) as *mut u8;
    ...
    fn receive(&self) -> Option<Vec<u8>> {
        ...
        let status = unsafe {
            self.rx_buffer.add(self.rx_offset) as *const u32
        };
        let status = unsafe { core::ptr::read_volatile(status) };
...
pub fn init() {
    ...
    for pci_dev in candidates.into_iter() {
        ...
        match unsafe { Rtl8139Device::new(pci_dev) } {
            Ok(device) => {
                ...
                log::info!("RTL8139: Registered as {}", iface);
            }
            Err(e) => {
    ...
    let devices = RTL8139_DEVICES.lock();
    log::info!("[RTL8139] Found {} device(s)", devices.len());
}
