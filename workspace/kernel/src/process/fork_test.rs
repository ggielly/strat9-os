...
fn spawn_user_program_task(
    ...
    let code_phys = user_as
        .translate(crate::arch::xshim::VirtAddr::new(USER_CODE_ADDR))
...
fn cow_test_refcount_unmap() -> bool {
    ...
    let phys = match aspace.translate(crate::arch::xshim::VirtAddr::new(COW_TEST_ADDR)) {
        Some(p) => p,
        None => {
...
fn cow_test_write_fault_copy() -> bool {
    ...
    let parent_phys = match aspace.translate(crate::arch::xshim::VirtAddr::new(COW_TEST_ADDR)) {
        Some(p) => p,
        None => {
    ...
    let child_phys = match child.translate(crate::arch::xshim::VirtAddr::new(COW_TEST_ADDR)) {
        Some(p) => p,
        None => {
