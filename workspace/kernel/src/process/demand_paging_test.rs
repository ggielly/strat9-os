...
fn test_fault_maps_and_refcount() -> bool {
    ...
    let phys = match aspace.translate(crate::arch::xshim::VirtAddr::new(DP_ADDR_A)) {
        Some(p) => p,
        None => return false,
    };
...
fn test_repeat_fault_same_page_no_leak() -> bool {
    ...
    let phys1 = match aspace.translate(crate::arch::xshim::VirtAddr::new(DP_ADDR_B)) {
        Some(p) => p,
        None => return false,
    };
    ...
    let phys2 = match aspace.translate(crate::arch::xshim::VirtAddr::new(DP_ADDR_B)) {
        Some(p) => p,
        None => return false,
    };
    if phys1 != phys2 {
