    ...
    loop {
        ...
        if ticks().saturating_sub(start_tick) > TIMEOUT_TICKS {
            crate::serial_println!(
                "[vfs-stat-test] FAIL: timeout waiting clock advance for '{}' (prev_ns={})",
                label,
                prev_ns
            );
            return false;
...
fn log_stat(tag: &str, path: &str, st: &vfs::FileStat) {
    ...
    crate::serial_println!(
        "[vfs-stat-test] {} path='{}' dev={} ino={} mode={:#o} nlink={} uid={} gid={} rdev={} atime={} mtime={} ctime={} size={}",
        tag,
        path,
        st.st_dev,
        st.st_ino,
...
fn assert_ge(name: &str, lhs: u64, rhs: u64) -> bool {
    if lhs < rhs {
        crate::serial_println!(
            "[vfs-stat-test] FAIL: {} expected >= {}, got {}",
            name,
            rhs,
            lhs
        );
...
fn run_vfs_stat_timestamp_suite() -> bool {
    ...
    crate::serial_println!("[vfs-stat-test] start suite base='{}'", base);

    if let Err(e) = vfs::mkdir(&base, 0o755) {
        crate::serial_println!("[vfs-stat-test] FAIL: mkdir('{}') => {:?}", base, e);
        return false;
    }

    if let Err(e) = vfs::create_file(&file, 0o644) {
        crate::serial_println!("[vfs-stat-test] FAIL: create_file('{}') => {:?}", file, e);
    ...
    let fd_file = match vfs::open(&file, vfs::OpenFlags::READ | vfs::OpenFlags::WRITE) {
        Ok(fd) => fd,
        Err(e) => {
            crate::serial_println!("[vfs-stat-test] FAIL: open rw file => {:?}", e);
    ...
    let mut st = match vfs::fstat(fd_file) {
        Ok(s) => s,
        Err(e) => {
            crate::serial_println!("[vfs-stat-test] FAIL: fstat create => {:?}", e);
    ...
    if !wait_clock_advance(anchor, "before read") {
        ...
        if STRICT_MODE {
            ok &= assert_gt("atime after read (strict)", a1, ts_nanos(&st).0);
            ok &= assert_eq_u64("mtime unchanged after read (strict)", m1, m0);
            ok &= assert_eq_u64("ctime unchanged after read (strict)", c1, c0);
        } else {
            ok &= assert_ge("atime after read", a1, ts_nanos(&st).0);
            ok &= assert_ge("mtime monotonic after read", m1, m0);
            ok &= assert_ge("ctime monotonic after read", c1, c0);
        }
        st = st1;
    }
    ...
    if !wait_clock_advance(prev, "before write") {
        ...
        crate::serial_println!("[vfs-stat-test] write bytes={}", wr);
        let st2 = vfs::fstat(fd_file).unwrap();
        log_stat("after write", &file, &st2);
        let (a2, m2, c2) = ts_nanos(&st2);
        if STRICT_MODE {
            ok &= assert_gt("mtime after write (strict)", m2, ts_nanos(&st).1);
            ok &= assert_gt("ctime after write (strict)", c2, ts_nanos(&st).2);
            ok &= assert_ge("atime monotonic after write", a2, ts_nanos(&st).0);
        } else {
            ok &= assert_ge("mtime after write", m2, ts_nanos(&st).1);
            ok &= assert_ge("ctime after write", c2, ts_nanos(&st).2);
        }
        st = st2;
    }
    ...
    if !wait_clock_advance(prev, "before truncate") {
        ...
        if let Err(e) = vfs::truncate(&file, 4) {
            crate::serial_println!("[vfs-stat-test] FAIL: truncate => {:?}", e);
            ok = false;
        } else {
            let st3 = vfs::fstat(fd_file).unwrap();
            log_stat("after truncate", &file, &st3);
            if STRICT_MODE {
                ok &= assert_gt("mtime after truncate (strict)", ts_nanos(&st3).1, ts_nanos(&st).1);
                ok &= assert_gt("ctime after truncate (strict)", ts_nanos(&st3).2, ts_nanos(&st).2);
            } else {
                ok &= assert_ge("mtime after truncate", ts_nanos(&st3).1, ts_nanos(&st).1);
                ok &= assert_ge("ctime after truncate", ts_nanos(&st3).2, ts_nanos(&st).2);
            }
            st = st3;
        }
    }
    ...
    if !wait_clock_advance(prev, "before chmod") {
        ...
        if let Err(e) = vfs::chmod(&file, 0o600) {
            crate::serial_println!("[vfs-stat-test] FAIL: chmod => {:?}", e);
            ok = false;
        } else {
            let st4 = vfs::fstat(fd_file).unwrap();
            log_stat("after chmod", &file, &st4);
            if STRICT_MODE {
                ok &= assert_gt("ctime after chmod (strict)", ts_nanos(&st4).2, ts_nanos(&st).2);
                ok &= assert_eq_u64("mtime unchanged after chmod (strict)", ts_nanos(&st4).1, ts_nanos(&st).1);
            } else {
                ok &= assert_ge("ctime after chmod", ts_nanos(&st4).2, ts_nanos(&st).2);
            }
            st = st4;
        }
    }
    ...
    if !wait_clock_advance(prev, "before link") {
        ...
        if let Err(e) = vfs::link(&file, &hard) {
            crate::serial_println!("[vfs-stat-test] FAIL: link => {:?}", e);
            ok = false;
        } else {
            ...
            ok &= st5.st_nlink >= 2;
            if STRICT_MODE {
                ok &= assert_gt("ctime after link (strict)", ts_nanos(&st5).2, ts_nanos(&st).2);
            } else {
                ok &= assert_ge("ctime after link", ts_nanos(&st5).2, ts_nanos(&st).2);
            }
            st = st5;
        }
    }
    ...
    if !wait_clock_advance(prev, "before rename") {
        ...
        if let Err(e) = vfs::rename(&file, &file2) {
            crate::serial_println!("[vfs-stat-test] FAIL: rename => {:?}", e);
            ok = false;
        } else {
            let st6 = vfs::stat_path(&file2).unwrap();
            log_stat("after rename(dst)", &file2, &st6);
            if STRICT_MODE {
                ok &= assert_gt("ctime after rename (strict)", ts_nanos(&st6).2, ts_nanos(&st).2);
            } else {
                ok &= assert_ge("ctime after rename", ts_nanos(&st6).2, ts_nanos(&st).2);
            }
    ...
    if let Err(e) = vfs::symlink("file2.txt", &sym) {
        ...
        if wait_clock_advance(prev, "before readlink") {
            ...
            crate::serial_println!("[vfs-stat-test] readlink('{}') => '{}'", sym, target);
    ...
    let fd_dir = match vfs::open(&base, vfs::OpenFlags::READ | vfs::OpenFlags::DIRECTORY) {
        Ok(fd) => fd,
        Err(e) => {
            crate::serial_println!("[vfs-stat-test] FAIL: open dir => {:?}", e);
            let _ = vfs::close(fd_file);
            return false;
        }
    };
    let st_dir_before = match vfs::fstat(fd_dir) {
        Ok(s) => s,
        Err(e) => {
            crate::serial_println!("[vfs-stat-test] FAIL: fstat dir before readdir => {:?}", e);
...
extern "C" fn vfs_stat_test_main() -> ! {
    crate::serial_println!("[vfs-stat-test] task start");
    let _ = run_vfs_stat_timestamp_suite();
    crate::serial_println!("[vfs-stat-test] task done");
...
pub fn create_vfs_stat_test_task() {
    if let Ok(task) = Task::new_kernel_task_with_stack(
        ...
        crate::serial_println!("[vfs-stat-test] failed to create task");
    }
}
