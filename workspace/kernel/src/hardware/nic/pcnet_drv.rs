...
impl Ports {
    ...
    fn read8(&self, offset: u16) -> u8 {
        unsafe { crate::arch::legacy_port::Port::new(self.io_base + offset).read() }
    }
    ...
    fn write8(&mut self, offset: u16, value: u8) {
        unsafe { crate::arch::legacy_port::Port::new(self.io_base + offset).write(value) }
    }
    ...
    fn write_rap(&mut self, val: u32) {
        unsafe { crate::arch::legacy_port::Port::new(self.io_base + 0x14).write(val) }
    }

    /// Reads rdp.
    fn read_rdp(&self) -> u32 {
        unsafe { crate::arch::legacy_port::Port::new(self.io_base + 0x10).read() }
    }

    /// Writes rdp.
    fn write_rdp(&mut self, val: u32) {
        unsafe { crate::arch::legacy_port::Port::new(self.io_base + 0x10).write(val) }
    }

    /// Reads bdp.
    fn read_bdp(&self) -> u32 {
        unsafe { crate::arch::legacy_port::Port::new(self.io_base + 0x1C).read() }
    }

    /// Writes bdp.
    fn write_bdp(&mut self, val: u32) {
        unsafe { crate::arch::legacy_port::Port::new(self.io_base + 0x1C).write(val) }
    }
