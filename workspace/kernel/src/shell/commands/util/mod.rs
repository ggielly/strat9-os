...
pub fn cmd_watch(args: &[String]) -> Result<(), ShellError> {
    ...
    loop {
        ...
        crate::shell::output::start_capture();
        let registry = crate::shell::commands::CommandRegistry::new();
        crate::shell::scripting::set_last_exit(0);
        if let Some(pipeline) = crate::shell::parser::parse_pipeline(&cmd_line) {
            for stage in &pipeline.stages {
                let _ = registry.execute(&stage.command);
            }
        }
        let captured = crate::shell::output::take_capture();
        if let Ok(s) = core::str::from_utf8(&captured) {
            crate::shell_print!("{}", s);
        }
