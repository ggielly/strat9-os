...
| Périphériques | Le DTB fournit PLIC, PCI ECAM et les plages MMIO ; l’accès configuration ECAM, le scan borné du bus 0 et la découverte des nœuds VirtIO‑MMIO sont implémentés. | Aucun device VirtIO n’est fourni par la ligne QEMU actuelle et aucun driver de transport n’est encore branché. |
...
Le contrat d'entrée OpenSBI, la découverte DTB, l’initialisation buddy, Sv39, le trap handler S‑mode, le PLIC, le timer à 100 Hz, le scan PCI ECAM et la découverte des nœuds VirtIO‑MMIO sont matérialisés. Les 16 plages MMIO découvertes dans le DTB sont mappées avant l’activation de `satp`. L’étape suivante est l’ajout d’un device VirtIO‑MMIO dans QEMU et son premier driver, puis l’espace utilisateur.
